 ...
 * 
 * Serves menu feed to Google Merchant Center
 * Supports JSON and XML formats for menu catalog
 * 
 * Deployment:
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Get business details for feed metadata
    ...
    const checkoutBaseUrl = businessData?.website_url || 'https://yourpos.com';
    const businessName = businessData?.name || 'RMP POS';

    // Generate feed based on format
    const feed = format === 'json'
      ? generateJSONFeed(menuItems, businessId, businessName, checkoutBaseUrl)
      : generateXMLFeed(menuItems, businessId, businessName, checkoutBaseUrl);
...
/**
 * Generate JSON feed format for Google Merchant Center
 */
...
/**
 * Generate XML feed format (Atom format)
 */
function generateXMLFeed(
  ...
  xml += `  <id>tag:rmp-pos:${escapeXml(businessId)}</id>\n`;
  ...
  for (const item of items) {
    ...
    xml += `    <category term="${escapeXml(item.category || 'Uncategorized')}" />\n`;
    ...
    if (item.image_url) {
      xml += `        <img src="${escapeXml(item.image_url)}" alt="${escapeXml(item.name)}" />\n`;
    }

    xml += `        <p><strong>Price:</strong> $${item.price.toFixed(2)}</p>\n`;

    if (item.prep_time_minutes) {
      xml += `        <p><strong>Prep Time:</strong> ${item.prep_time_minutes} minutes</p>\n`;
    }

    if (item.dietary_restrictions && item.dietary_restrictions.length > 0) {
      xml += `        <p><strong>Dietary Info:</strong> ${escapeXml(item.dietary_restrictions.join(', '))}</p>\n`;
    }
